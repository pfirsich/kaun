...
VertexBuffer* Mesh::hasAttribute(AttributeType attrType) const
{
    for (auto& vBuffer : mVertexBuffers) {
        if (vBuffer->getVertexFormat().hasAttribute(attrType))
            return vBuffer.get();
    }
    return nullptr;
}
    ...
    for (auto& vData : mVertexBuffers) {
        ...
        for (size_t i = 0; i < attributes.size(); ++i) {
            ...
            glVertexAttribPointer(location, attr.alignedNum, static_cast<GLenum>(attr.dataType),
                attr.normalized ? GL_TRUE : GL_FALSE, format.getStride(),
...
void Mesh::draw(size_t instanceCount)
{
    ...
    // A lof of this can go wrong if someone compiles this Mesh without an index buffer attached,
    // then attaches one and compiles it with another shader, while both are in use
    GLenum mode = static_cast<GLenum>(mMode);
    if (mIndexBuffer != nullptr) {
        GLenum indexType = static_cast<GLenum>(mIndexBuffer->getDataType());
        if (instanceCount > 0) {
            glDrawElementsInstanced(
                mode, mIndexBuffer->getNumIndices(), indexType, nullptr, instanceCount);
        } else {
            glDrawElements(mode, mIndexBuffer->getNumIndices(), indexType, nullptr);
        }
    } else {
        // If someone had the great idea of having multiple VertexBuffer objects attached and
        // changing their size after attaching this might break
        size_t size = 0;
        if (mVertexBuffers.size() > 0)
            size = mVertexBuffers[0]->getNumVertices();
        if (instanceCount > 0) {
...
void Mesh::normalize(bool rescale)
{
    ...
    glm::mat4 t(1.0);
    if (rescale)
        t = glm::scale(t, glm::vec3(1.0f / bSphere.second));
...
Mesh* Mesh::box(float width, float height, float depth, const VertexFormat& format)
{
    ...
    static uint8_t indices[6] = { 0, 1, 2, 0, 2, 3 };
...
Mesh* Mesh::sphere(
    float radius, int slices, int stacks, bool cubeProjectionTexCoords, const VertexFormat& format)
{
    ...
    but I did it like this to show that it's possible to do it like this without everything going up
    in flames
     */
    ...
    for (int stack = 0; stack < stacks; ++stack) {
        ...
        for (int slice = 0; slice < slices; ++slice) {
            ...
            if (cubeProjectionTexCoords) {
                ...
                texCoord.set(index++,
                    glm::vec2(
                        (v.x / glm::abs(v.z) + 1.0f) / 2.0f, (v.y / glm::abs(v.z) + 1.0f) / 2.0f));
            } else {
                texCoord.set(index++,
                    glm::vec2(sliceAngle / 2.0f / glm::pi<float>(), stackAngle / glm::pi<float>()));
            }
        }
    }
...
Mesh* loadTinyObj(const tinyobj::attrib_t& attrib, const std::vector<tinyobj::shape_t>& shapes,
    const std::vector<tinyobj::material_t>& materials, const VertexFormat& format)
{
...
Mesh* Mesh::objFile(const uint8_t* buffer, size_t size, const VertexFormat& format)
{
    ...
    if (ret) {
        if (err.length() > 0)
            LOG_WARNING("Warning loading object file: %s", err.c_str());
    } else {
...
Mesh* Mesh::objFile(const std::string& filename, const VertexFormat& format)
{
    ...
    std::string err;
    bool ret
        = tinyobj::LoadObj(&attrib, &shapes, &materials, &err, filename.c_str(), nullptr, true);
