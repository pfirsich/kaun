...
public:
    ...
    RenderState()
        : mDepthWrite(true)
        , mDepthFunc(DepthFunc::LEQUAL)
        , mCullFaces(FaceDirections::BACK)
        , mFrontFace(FaceOrientation::CCW)
        , mBlendEnabled(false)
    ...
    DepthFunc getDepthTest() const
    {
        return mDepthFunc;
    }
    void setDepthTest(DepthFunc func = DepthFunc::LEQUAL)
    {
        mDepthFunc = func;
    }

    FaceDirections getCullFaces() const
    {
        return mCullFaces;
    }
    void setCullFaces(FaceDirections dirs = FaceDirections::BACK)
    {
        mCullFaces = dirs;
    }

    FaceOrientation getFrontFace() const
    {
        return mFrontFace;
    }
    void setFrontFace(FaceOrientation ori)
    {
        mFrontFace = ori;
    }
    ...
    std::pair<BlendFactor, BlendFactor> getBlendFactors() const
    {
        return std::make_pair(mBlendSrcFactor, mBlendDstFactor);
    }
    ...
    void setBlendFactors(std::pair<BlendFactor, BlendFactor> factors)
    {
        setBlendFactors(factors.first, factors.second);
    }
    void setBlendSrcFactor(BlendFactor src)
    {
        mBlendSrcFactor = src;
    }
    void setBlendDstFactor(BlendFactor dst)
    {
        mBlendDstFactor = dst;
    }

    BlendEq getBlendEquation() const
    {
        return mBlendEquation;
    }
    void setBlendEquation(BlendEq eq)
    {
        mBlendEquation = eq;
    }
