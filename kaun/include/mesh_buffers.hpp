...
public:
    ...
    GLBuffer(GLenum target, void* data, size_t size, UsageHint usage)
        ...
        , mSize(size)
        , mUsage(usage)
        , mData(reinterpret_cast<VBODataType*>(data))
        , mBufferObject(0)
        , mLastUploadedSize(0)
        , mUploadCount(0)
    {
    ...
    // http://hacksoflife.blogspot.de/2015/06/glmapbuffer-no-longer-cool.html - Don't implement
    // map()?
...
// That means that they have full ownership of the data, you may reallocate it or get access to the
// data they points to but never change the data they point to
...
public:
    // The vertex formats will be copied, so it's not possible to to dangerous shenanigans by
    // changing them after they were used
    ...
    VertexBuffer(
        const VertexFormat& format, size_t numVertices, UsageHint usage = UsageHint::STATIC)
        : GLBuffer(GL_ARRAY_BUFFER, nullptr, 0, usage)
        , mVertexFormat(format)
    ...
    VertexBuffer(const VertexFormat& format, void* data, size_t numVertices,
        UsageHint usage = UsageHint::STATIC)
...
public:
    ...
    IndexBuffer(uint16_t* data, size_t num, UsageHint usage = UsageHint::STATIC)
        ...
        , mDataType(IndexBufferType::UI16)
        , mNumIndices(num)
    {
    }
    IndexBuffer(uint32_t* data, size_t num, UsageHint usage = UsageHint::STATIC)
        ...
        , mDataType(IndexBufferType::UI32)
        , mNumIndices(num)
    {
    }
    ...
    IndexBufferType getDataType() const
    {
        return mDataType;
    }
