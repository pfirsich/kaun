...
public:
    ...
    {
        // this normalization is not necessary mathematically, but technically (because of floating
        // point numbers) it is
    ...
    glm::mat4 getMatrix() const
    {
        if (mMatrixDirty) {
            mMatrix = glm::scale(glm::translate(glm::mat4(1.0f), mPosition)
                    * glm::mat4_cast(glm::conjugate(mQuaternion)),
                mScale);
            mMatrixDirty = false;
        }
        return mMatrix;
    ...
    // this works just as gluLookAt, so may put in "world space up" (this might not work sometimes,
    // but makes everything a lot easier most of the time) it also means, that the negative z-axis
    // is aligned to face "at"
