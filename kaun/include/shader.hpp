...
public:
    ...
    enum class Status { EMPTY, UNLINKED, LINKED, COMPILE_SHADER_FAILED, LINK_FAILED };
...
private:
    ...
    static std::string_view globalShaderPreamble;
    static std::string_view fragmentShaderPreamble;
    static std::string_view vertexShaderPreamble;
    static std::string_view geometryShaderPreamble;
...
public:
    ...
    {
        return compileString(frag, Type::FRAGMENT) && compileString(vert, Type::VERTEX) && link();
    }
    ...
    {
        return compileFile(fragPath, Type::FRAGMENT) && compileFile(vertPath, Type::VERTEX)
            && link();
    }
