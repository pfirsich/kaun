...
void Shader::retrieveUniformInfo()
{
    ...
    for (int i = 0; i < activeUniformCount; ++i) {
        ...
        if (length > 0) {
            mUniformInfo[name]
                = UniformInfo(i, size, static_cast<UniformInfo::UniformType>(type), name);
        }
    }
    delete[] name;
}
...
Shader::UniformLocation Shader::getAttributeLocation(
    const std::string& name, bool logNotFound) const
{
