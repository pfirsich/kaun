...
std::unordered_map<GLenum, const char*> debugSourceName
    = { { GL_DEBUG_SOURCE_API, "api" }, { GL_DEBUG_SOURCE_SHADER_COMPILER, "shader_compiler" },
          { GL_DEBUG_SOURCE_WINDOW_SYSTEM, "window_system" },
          { GL_DEBUG_SOURCE_THIRD_PARTY, "third_party" },
          { GL_DEBUG_SOURCE_APPLICATION, "application" }, { GL_DEBUG_SOURCE_OTHER, "other" } };

std::unordered_map<GLenum, const char*> debugTypeName = { { GL_DEBUG_TYPE_ERROR, "error" },
    { GL_DEBUG_TYPE_DEPRECATED_BEHAVIOR, "deprecated_behavior" },
    { GL_DEBUG_TYPE_UNDEFINED_BEHAVIOR, "undefined_behavior" },
    { GL_DEBUG_TYPE_PERFORMANCE, "performance" }, { GL_DEBUG_TYPE_PORTABILITY, "portability" },
    { GL_DEBUG_TYPE_OTHER, "other" }, { GL_DEBUG_TYPE_MARKER, "marker" },
    { GL_DEBUG_TYPE_PUSH_GROUP, "push_group" }, { GL_DEBUG_TYPE_POP_GROUP, "pop_group" } };
    ...
    const GLchar* message, const void* userParam)
{
    if (severity == GL_DEBUG_SEVERITY_NOTIFICATION)
        return;
    LOG_DEBUG("GL Debug message - source: %s, type: %s, severity: %s, message: %s",
        debugSourceName[source], debugTypeName[type], debugSeverityName[severity], message);
}
...
void init()
{
    ...
    defaultVertexFormat.add(kaun::AttributeType::POSITION, 3, kaun::AttributeDataType::F32)
