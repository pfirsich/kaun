...
namespace kaun {
    ...
    VertexAttribute::VertexAttribute(AttributeType attrType, int num,
                                     AttributeDataType dataType, int offset,
                                     bool normalized, unsigned int divisor) :
                type(attrType), num(num), alignedNum(num), dataType(dataType), offset(offset),
